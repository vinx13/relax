...
class StaticRegionExtractor : public ExprVisitor {
 public:
  ...
  void SummarizeStaticRegion() {
    ...
    // Emit other nodes
    static_region_builder->Emit(Call(call_builtin_op_,
                                     {builtin_cuda_graph_begin_capture_, Tuple(Array<Expr>{})},
                                     Attrs(DefaultBuiltinAttrs())));
    ...
    auto graph = static_region_builder->Emit(
        Call(call_builtin_op_, {builtin_cuda_graph_end_capture_, Tuple(Array<Expr>{})},
             Attrs(DefaultBuiltinAttrs())));
  ...
  const ExternFunc builtin_cuda_graph_begin_capture_{"vm.builtin.cuda_graph_begin_capture"};
  const ExternFunc builtin_cuda_graph_end_capture_{"vm.builtin.cuda_graph_end_capture"};
};
...
class CUDAGraphRewriter : public ExprMutator {
 public:
  ...
  void MakeGetCapturedCUDAGraph(const GlobalVar capturing_func) {
    auto attrs = DefaultBuiltinAttrs();
    attrs->require_ctx = true;
    return Call(call_builtin_op_, {builtin_get_captured_cuda_graph_, Tuple({gv})}, Attrs(attrs),
                {TupleType({ObjectType(), GetStaticType(region.func->ret_struct_info)})});
  }
  ...
  BindingBlock VisitBindingBlock_(const BindingBlockNode* binding_block) final {
    ...
    if (auto it = binding_block_to_regions_.find(binding_block);
      ...
      for (const auto& region : it->second) {
        ...
        auto graph_tensors = builder_->Emit(MakeGetCapturedCUDAGraph(gv));
  ...
  void VisitBinding_(const VarBindingNode* binding) final {
    if (auto it = graph_launch_point_.find(binding->var.get()); it != graph_launch_point_.end()) {
      // Emit launch graph
      builder_->Emit(Call(call_builtin_op_,
                          {builtin_cuda_graph_launch_, Tuple({GetRef<Var>(it->second)})},
                          Attrs(DefaultBuiltinAttrs())));
      return;
